...
\section{Results} 
\subsection{Consensus binding site detection}
The NSP13 helicase of SARS-CoV-2 has been identifed as vital for viral replication and thus is considered a promising target for antiviral drugs \cite{Berta_2021}. Before we can screen ligand libraries for potential inhibitorrs, the druggability of surface of the NSP13 helicase needs to be investigated to determine ligand binding sites. For a more precise identification three different crystal structures (PDB codes: 6ZSL, 5RM2) were analysed using three different tools which follow distinct computational methods (geometry-based, machine-learning-based and docking-based). The calculated binding hot spots were visualised in PyMol (Version 2.5.7, \textcite{PyMol_endnote}) and overlapping ones were merged throguh visual examination to obtain a consensus binding pocket.
Overall, the hot spots calculated with Fpocket which overlapped between the three crystal structures all ranked first amon the 29-38 calculated binding sites. Their \acp{DScore} were 0.647 for 6ZSL, 0.383 for 5RM2, and 0.875 for 5RME. Also, the respective overlapping binding sites calculated with PrankWeb ranked either first (5RM2 and 5RME) or second (6ZSL) among the 10-11 calculated binding sites. Therefore, their Scores and probability was also very high. 
Interestingly, for the crystal structure 5RME a second binding hot spot was detected in close proximity to the 1B domain. Nvertheless, 
The resulting consensus binding pocket \textbf {HIER KOMMT EIN BILD HIN} is located in the ATP binding pocket in between the two RecA-like domains RecA1 and RecA2. %bzw. RecA1 and RecA2?



It is composed of 20 amino acid residues (Table \ref{tab:residues}) and has a volume of $1,000 {\AA}^3$. The residues are mainly hydrophobic and aromatic, but also contain polar and positively charged residues. The pocket is located in the interior of the protein and is thus not directly accessible from the outside.   
