...
\usepackage{booktabs} %  Selina
\usepackage{makecell} % Selina 
...
\section*{Abbreviations}
    ...
    \acro{ECBD}{European Chemical Biology Database}
...
%The resulting heatmap of the compounds fulfilling this rule is shown in Figure \ref{hc_properties}.
...
\begin{figure}[htp]
	...
	\subcaptionbox{%
		\label{subfig:domains}}{\includegraphics[width=0.3\textwidth]{consens_domain_big.pdf}}
	\subcaptionbox{%
		\label{subfig:pocket}}{\includegraphics[width=0.5\textwidth]{pocket_w_labels.pdf}}
	\caption{\textbf{Consensus binding pocket of the NSP13 helicase.} In (\subref{subfig:pocket}) you can see the consensus binding pocket (orange) detected with \textit{Fpocket}, \textit{PrankWeb}, and \textit{FTMap}. The binding pocket is located in the ATP binding pocket in between the two RecA-like domains A1 and A2. In (\subref{subfig:domains}) the important amino acid residues of the consensus binding site are showcased. In green, you can see the residues that are part that are part of the consensus binding pocket. In blue, you can see the residues that are part of the consensus binding pocket and are also found by \textcite{Berta_2021} as part of the ATP binding site. The amino acid residues only found by \textcite{Berta_2021} are depicted in pink. Additionally, amino residues that interact with our top three scoring ligands which were identified with \textit{Glide} are coloured blue. The amino acid residues that are forming H-bonds with our top scoring ligand are labelled.}\label{consensus_binding_pocket}
\end{figure}
\FloatBarrier
...
\subsection{Clustering of compounds}
Comparison of the results from the docking simulations of the three paired teams ("Warsaw1", "Sorbonne5", and "Heidelberg") was performed by clustering the top 30 scoring compounds of the teams based on their structural similarity (Figure \ref{hc_properties} (A)). Here, we can see that the three compounds on ranks 4,5, and 6 from "Sorbonne5" are most dissimilar to each other. Overall the compounds cannot clearly be divided into clusters. This is also reflected in the heatmap that was created based on the physico-chemical properties of the compounds (Figure \ref{hc_properties} (B)). Nevertheless, 22 out of the 30 investigated compounds fulfill Lipinski's Rule of Five. Interestingly, all of the compounds that did not fulfill Lipinski's rule share the property that they contain more than 10 \ac{HBA}. Of these compounds, five were detected by team Heidelberg, including our top scoring ligand. The other three compounds came from "Sorbonne5" and none came from "Warsaw1". These eight compounds that were not drug-like were removed in the following steps.
...
%The analysis of natural compound datasets by \citeauthor{pu2019toxpred} revealed a bimodal distribution in the \ac{SAscore}, with peaks around 3 and 5. Furthermore the very low Tox-Score of the top scorer can also be explained by the fact that it is a naturally occuring molecule in the human body. \\
...
\begin{figure}[h]
	...
	\captionsetup[subfigure]{skip=-15pt,position=top,labelfont=bf,labelformat=parens,singlelinecheck=false}
	\subcaptionbox{%
		\label{subfig:top1_glide_diffdock}}{\includegraphics[scale=0.33]{Top1_Glide_Rank5}}
	\subcaptionbox{%
		\label{subfig:top2_glide_diffdock}}{\includegraphics[scale=0.33]{Top2_Glide_Rank5}}
	\subcaptionbox{%
		\label{subfig:top3_glide_diffdock}}{\includegraphics[scale=0.33]{Top3_Glide_Rank5}}
...
\begin{figure}[htp]
	\centering
	\captionsetup[subfigure]{skip=-15pt,position=top,labelfont={bf},format=plain,textfont=rm, labelformat=parens, singlelinecheck=false,size=large}
	\subcaptionbox{%
		\label{subfig:lipinsky_heatmap}}{\includegraphics[width=0.7\textwidth]{all2_tops_hc.pdf}}
	\subcaptionbox{%
		\label{subfig:all_tops_hc}}{\includegraphics[width=0.9\textwidth]{Lipinsky_heatmap.pdf}}
	\caption{\textbf{Comparison of the (a) structural and (b) physico-chemical properties of the top scoring compounds from Sorbonne5 (H), Warsaw1 (W), and Heidelberg (H).} The first heatmap (a) is based on the distances of the compounds calculated using the Tanimoto coefficient. In (b) the physic-chemical properties were determined using Open Babel descriptors (\acl{HBD}, \acl{HBA}, \acl{MW}, \acl{logP}). Here, only compounds fulfilling Lipinski's rule of five are shown.}\label{hc_properties}
\end{figure}
